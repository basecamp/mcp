...
haiku       12/12     12/12     12/12     19199      291        $0.0165 (estimated)
```

Under two cents ($0.0165, estimated: the CLI backend reports no token counts,
so they are derived at ~4 characters per token and the records say so) of
model spend proves the whole loop turns and the cost story is real. A capable model clears this small, unambiguous corpus cleanly:
